...
static void _config_init_reload(struct mqtt3_config *config)
{
	...
	if(config->topic_change_keepalive) _mosquitto_free(config->topic_change_keepalive);
...
void mqtt3_config_init(struct mqtt3_config *config)
{
	...
	config->use_subhier_accelerator = false;
	config->topic_change_keepalive = NULL;
	config->topic_notice_offline = NULL;
		...
		if(buf[0] != '#' && buf[0] != 10 && buf[0] != 13){
			...
			if(token){
				if(!strcmp(token, "acl_file")){
					...
					if(_conf_parse_bool(&token, "use_epoll", &config->use_epoll, saveptr)) return MOSQ_ERR_INVAL;
				}else if(!strcmp(token, "use_subhier_accelerator")){
					if(_conf_parse_bool(&token, "use_subhier_accelerator", &config->use_subhier_accelerator, saveptr)) return MOSQ_ERR_INVAL;
				}else if(!strcmp(token, "use_bridge")){
					if(_conf_parse_bool(&token, "use_bridge", &config->with_bridge, saveptr)) return MOSQ_ERR_INVAL;
					...
					if(_conf_parse_string(&token, "topic_notice_online", &config->topic_notice_online, saveptr)) return MOSQ_ERR_INVAL;
				}else if(!strcmp(token, "topic_notice_offline")){
					if(_conf_parse_string(&token, "topic_notice_offline", &config->topic_notice_offline, saveptr)) return MOSQ_ERR_INVAL;
				}else if(!strcmp(token, "topic_dump_connection")){
