...
{
	int rc = 0;
	struct _mosquitto_subhier *child = NULL;
	struct _mosquitto_subhier *business_child = NULL;
	...
	db->subs.prev = NULL;
	db->subs.topic = "";
	db->subs.children = NULL;
	business_child = add_child_subhier_node(db, NULL, "", "");
	if(!business_child){
		_mosquitto_log_printf(NULL, MOSQ_LOG_ERR, "Error: Out of memory. for topic:\"\"");
		return MOSQ_ERR_NOMEM;
	}

	child = add_child_subhier_node(db, NULL, "$SYS", "$SYS");
	if(!child){
		subhier_clean(db, business_child);
		_mosquitto_log_printf(NULL, MOSQ_LOG_ERR, "Error: Out of memory. for topic: $SYS");
		return MOSQ_ERR_NOMEM;
	}
...
static void subhier_clean(struct mosquitto_db *db, struct _mosquitto_subhier *subhier)
{
	...
	while(subhier){
		...
		if(subhier->topic_key) _mosquitto_free(subhier->topic_key);
...
int mqtt3_db_message_insert(struct mosquitto_db *db, struct mosquitto *context, uint16_t mid, enum mosquitto_msg_direction dir, int qos, bool retain, struct mosquitto_msg_store *stored)
{
	...
	if((int)context->sock >= 0){
		tmp_mosq = NULL;
		HASH_FIND(hh_msg_sock, db->contexts_with_msg_by_sock, &context->sock, sizeof(mosq_sock_t), tmp_mosq);
		if(!tmp_mosq){	
			HASH_ADD(hh_msg_sock, db->contexts_with_msg_by_sock, sock, sizeof(context->sock), context);
			_mosquitto_log_printf(NULL, MOSQ_LOG_DEBUG, "[mqtt3_db_message_insert]Add context: %s into msg hash table.", context->id);
		}
	}
