...
{
	if(db->config->use_subhier_accelerator){
		return mqtt3_sub_add_fast(db, context, sub, qos, root);
	}
...
{
	if(db->config->use_subhier_accelerator){
		return mqtt3_sub_remove_fast(db, context, sub, root);
	}
...
{
	if(db->config->use_subhier_accelerator){
		return mqtt3_db_messages_queue_fast(db, source_id, topic, qos, retain, stored);
	}
...
{
	if(db->config->use_subhier_accelerator){
		return mqtt3_subs_clean_session_fast(db, context);
	}
