...
#  ifdef WITH_WEBSOCKETS
	if(mosq->wsi){
		...
		return _mosquitto_packet_write(NULL, mosq);
	}
#  else
	_mosquitto_log_printf(NULL, MOSQ_LOG_DEBUG, "[_mosquitto_packet_queue] will call _mosquitto_packet_write, moq(%s)", mosq->id);
	return _mosquitto_packet_write(NULL, mosq);
#  endif
#else
...
#endif
{
	...
	if((int)mosq->sock >= 0){
		...
		HASH_FIND(hh_msg_sock, db->contexts_with_msg_by_sock, &mosq->sock, sizeof(mosq_sock_t), tmp_mosq);
		if(tmp_mosq){
			HASH_DELETE(hh_msg_sock, db->contexts_with_msg_by_sock, tmp_mosq);
		}
#endif
...
#ifdef WITH_BROKER
int _mosquitto_packet_write(struct mosquitto_db *db, struct mosquitto *mosq)
#else
		...
		while(packet->to_process > 0){
			...
			if(write_length > 0){
				...
				if(errno == EAGAIN || errno == COMPAT_EWOULDBLOCK){
					pthread_mutex_unlock(&mosq->current_out_packet_mutex);
#ifdef WITH_BROKER
					if(db && db->config->use_epoll){
						return MOSQ_ERR_SOCKET_EAGAIN;
					}else{
						return MOSQ_ERR_SUCCESS;
					}
#else
